...
import static com.example.visualkhh.myapplication.view.util.PaintUtil.getDistance;
import static com.example.visualkhh.myapplication.view.util.PaintUtil.measureText;
...
public class PaintView extends View {
    ...
    public void addText(String text, float x, float y, int gravity) {
        ...
        switch (gravity) {
            case Constant.TextGravity.CENTER:
                x = (mWidth - textRect.width()) / 2;
                y = (mHeight + textRect.height()) / 2;
                break;
            case Constant.TextGravity.CENTER_HORIZONTAL:
                x = (mWidth - textRect.width()) / 2;
                break;
            case Constant.TextGravity.CENTER_VERTICAL:
                y = (mHeight + textRect.height()) / 2;
                break;
        }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_MOVE:
                //文字不在输入时，单点移动
                if (event.getPointerCount() == Constant.GestureFingers.SINGLE && !bGestureMoving) {
                    LogUtil.d("ACTION_SINGLE_MOVE");
                    touchMove(x, y);
                }
                //文字不在输入时，多点移动
                else if (event.getPointerCount() == Constant.GestureFingers.DOUBLE && bGestureEnable) {
        ...
        switch (mGestureState) {
            case Constant.GestureState.DRAG:
//                setDragMode();
                ...
                break;
            case Constant.GestureState.ZOOM:
//                setZoomMode();
                ...
                break;
            case Constant.GestureState.NONE:
